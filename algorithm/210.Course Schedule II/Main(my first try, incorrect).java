...
public class Main {
    public static List<Integer> mergeOrder(List< List<Integer>> segments) {
        if (segments.isEmpty()) {
            return new ArrayList<>();
        }
        List<Integer> order = segments.get(0);
        segments.remove(0);
        List<Integer> next;
        List<Integer> tmp;
        HashMap<Integer, Integer> idxMap1 = new HashMap<>();
        HashMap<Integer, Integer> idxMap2 = new HashMap<>();
        List<Integer> common = new ArrayList<>();
        List< List<Integer>> unconnected = new ArrayList<>();
        int passCount = 0;
        while (!segments.isEmpty()) {
            next = segments.get(0);
            if (next.size() > order.size()) {
                tmp = order;
                order = next;
                next = tmp;
            }
            if (order.get(0).equals(next.get(next.size() - 1))) {
                next.remove(next.size() - 1);
                next.addAll(order);
                order = next;
                segments.remove(0);
                passCount = 0;
            } else if (next.get(0).equals(order.get(order.size() - 1))) {
                order.remove(order.size() - 1);
                order.addAll(next);
                segments.remove(0);
                passCount = 0;
            } else {
                int j, cur, m = 0, n = 0, curCommon;
                for (j = 0; j < order.size(); j++) {
                    idxMap1.put(order.get(j), j);
                }
                for (j = 0; j < next.size(); j++) {
                    cur = next.get(j);
                    if (idxMap1.containsKey(cur)) {
                        common.add(cur);
                    }
                    idxMap2.put(cur, j);
                }
//                remove next and append next to tail to try again,
//                if passCount == segments.size(), then order is unconnected
                if (common.isEmpty()) {
                    passCount++;
                    segments.remove(0);
                    segments.add(next);
                    if (passCount == segments.size()) {
                        unconnected.add(order);
                        order = segments.get(0);
                        segments.remove(0);
                    }
                    continue;
                }
                j = 1;
                while (j < common.size()) {
                    if (idxMap1.get(common.get(j)) < idxMap1.get(common.get(j - 1))) {
                        return null;
                    }
                    j++;
                }
                ...
                j = 0;
                while (m < order.size() || n < next.size()) {
                    curCommon = j < common.size() ? common.get(j) : -1;
                    while (m < order.size() && order.get(m) != curCommon) {
                        r.add(order.get(m));
                        m++;
                    }
                    while (n < next.size() && next.get(n) != curCommon) {
                        r.add(next.get(n));
                        n++;
                    }
                    if (curCommon != -1) {
        ...
        int i = 0;
        while (i < unconnected.size()) {
            order.addAll(unconnected.get(i));
            i++;
        }
        return order;
    }
    public static boolean findSegments(HashMap<Integer, List<Integer>> graph, int start, List<List <Integer>> segments) {
    ...
    public static int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        for (Integer start : starts) {
            hasOrder = findSegments(graph, start, segments);
            if (!hasOrder) {
                return new int[]{};
            }
        }
        System.out.println(Arrays.toString(segments.toArray()));
        List<Integer> order = mergeOrder(segments);
        if (order == null) {
            return new int[]{};
        }
        ans.addAll(order);
        return ans.stream().mapToInt(it->it).toArray();
    }
