...
public class Main {
    public static boolean rankCourses(HashMap<Integer, List<Integer>> graph, int start, Set<Integer> taken, Set<Integer> coursesInPre, int[] inDeg, int[] rank) {
        ...
        if (graph.containsKey(start)) {
            ...
            for (Integer next : nexts) {
                ...
                if (rank[next] == 0) {
                    rank[next] = rank[start] + 1;
                } else {
                    int curRank = rank[start] + 1;
                    if (curRank > rank[next]) {
                        rank[next] = curRank;
                    }
                }
                if (inDeg[next] == 1) {
                    if (!rankCourses(graph, next, taken, coursesInPre, inDeg, rank)) {
                        return false;
                    }
    ...
    public static int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        for (Integer start : starts) {
            rank[start] = 1;
            if (!rankCourses(graph, start, taken, coursesInPre, inDeg, rank)) {
                return new int[]{};
            }
            taken.clear();
        }
        if (!coursesInPre.isEmpty()) {
            return new int[]{};
        }
        ...
        Collections.sort(ans, Comparator.comparingInt(it -> rank[it]));
